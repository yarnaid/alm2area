...
from alm2area.forms import AlmForm, MaskForm, MapImageForm, PointSourceForm, MapZoneCutFrom, HiddenErrorList, SmoothForm
...
def get_cut_gif_params(cuts, map_to_gif_params):
    ...
    cuts_gif_params['gif_name'] = get_name_in_media(dir=settings.MEDIA_ALM2AREA_IMAGE, suffix='map.gif')
...
def map_to_gif_params_init(map_to_gif_params, my_map):
    ...
    map_to_gif_params['gif_name'] = get_name_in_media(dir=settings.MEDIA_ALM2AREA_IMAGE, suffix='map.gif')
...
def alm_form_view(request):
    if request.method == 'POST':
        ...
        if forms['alm_form'].is_valid():
            ...
            alm_to_map_params['map_name'] = get_name_in_media(dir=settings.MEDIA_ALM2AREA_MAP, suffix='map.fit')
